...
def _run_program(reg_a, reg_b, reg_c, program):
    ...
    while instr_ptr < len(program):
        ...
        if instruction in COMBO_INSTRUCTIONS:
            operand = _get_combo_operand(
                reg_a, reg_b, reg_c, program, instr_ptr)
        elif instruction in LITERAL_INSTRUCTIONS:
            operand = program[instr_ptr+1]
        reg_a, reg_b, reg_c, instr_ptr, output = _process_instruction(
            reg_a, reg_b, reg_c, instruction, operand, instr_ptr, output)
    return output
